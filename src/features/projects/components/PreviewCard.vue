  ...
  >
    ...
    <div class="preview-card-content">
      <div class="preview-card-copys">
        ...
        <div class="preview-card-technologies notranslate" v-if="props.preview.technologies">
          <span 
            v-for="tech in props.preview.technologies" 
            :key="tech"
            class="preview-card-tech-pill"
          >
            {{ tech }}
          </span>
        </div>
...
.preview-card {
  ...
  &-tech-pill {
    ...
    @media (min-width: 1440px) { /* Approximate mixins.landscape-large fallback if needed, but let's just use regular CSS or check the mixin if possible. I'll use raw CSS just in case to avoid mixin errors if not imported, though mixins.hover is used above. */
      font-size: 11px;
      padding: 4px 10px;
    }
  }
